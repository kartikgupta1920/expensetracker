...
function ExpenseForm({setIsOpen, spentList , mon, setMon, editId, setSpentList}){
    ...
    const handleEdit = (e) => {
        ...
        const updated = spentList.map(item => {
            if(item.id == editId) {
                ...
                if(priceDifference < 0 && Math.abs(priceDifference) > mon) {
                    enqueueSnackbar("Price should not exceed the wallet balance", {variant: "warning"})
                    setIsOpen(false)
                    return {...item}
                }

                setMon(prev => prev + priceDifference)
